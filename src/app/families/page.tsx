...
export default async function FamiliesPage() {
  ...
  const families = (memberships ?? [])
    ...
    .map((m: any) => ({ role: m.role as string, family: (Array.isArray(m.families) ? m.families[0] : m.families) as { id: string; name: string } | null }))
    .filter(m => m.family)
    ...
    <div className="min-h-screen flex flex-col bg-bg-app">
      ...
      <main className="flex-1 overflow-y-auto">
        {families.length === 0 ? (
          ...
          <ul>
            {families.map(({ role, family }) => family && (
              <li key={family.id}>
                <Link
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-[17px] font-semibold text-text-primary">{family.name}</p>
