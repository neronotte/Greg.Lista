...
export default async function FamilyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: family }, { data: members }, navCounts] = await Promise.all([
    ...
    supabase
      .from('family_members')
      .select('role, user_id, joined_at, profile:profiles(id, email, display_name, avatar_url)')
      .eq('family_id', id),
    getNavCounts(),
  ])
    ...
    <div className="min-h-screen flex flex-col bg-bg-app">
      ...
      <main className="flex-1 overflow-y-auto">
        <section>
          ...
          {(members ?? []).map((m: any) => {
            const profile = (Array.isArray(m.profile) ? m.profile[0] : m.profile) as { id: string; email: string; display_name: string | null; avatar_url: string | null } | null
            if (!profile) return null
            const name = profile.display_name ?? profile.email.split('@')[0]
            return (
              <div key={m.user_id} className="flex items-center gap-3 bg-bg-surface px-4 py-3 border-b border-border">
                <Avatar name={name} email={profile.email} avatarUrl={profile.avatar_url} size={40} />
                <div className="flex-1 min-w-0">
                  <p className="text-base text-text-primary truncate">{name}</p>
                  <p className="text-sm text-text-secondary">{profile.email}</p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${m.role === 'owner' ? 'bg-brand-mid/20 text-brand-mid' : 'bg-bg-header text-text-secondary'}`}>
