...
export default function ListDetailActions({ listId, categories, items, fab, fabClassName, sectionOnly }: ListDetailActionsProps) {
    ...
    <>
      ...
      >
        <ItemForm
          ...
          onDone={() => { setSheetOpen(false); setEditItem(null) }}
        />
      </BottomSheet>

      <BottomSheet open={sessionSheetOpen} onClose={() => setSessionSheetOpen(false)} title="Avvia spesa">
        <form
          onSubmit={e => {
            e.preventDefault()
            startTransition(() => startSession(listId, supermarket || undefined))
          }}
          className="flex flex-col gap-4"
        >
          <div>
            <label className="block text-xs text-text-secondary mb-1">Supermercato (opzionale)</label>
            <input
              type="text"
              value={supermarket}
          ...
          <button type="submit" className="w-full py-3 rounded-lg bg-brand-bright text-white font-semibold text-base mt-2">
            Inizia
          </button>
        </form>
      </BottomSheet>
