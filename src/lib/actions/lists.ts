...
export async function createList(name: string, visibility: Visibility, familyId?: string) {
  ...
  const { data, error } = await supabase
    .from('lists')
    .insert({ name, visibility, owner_id: user.id, family_id: familyId ?? null })
    .select()
    .single()
...
export async function updateList(id: string, name: string, visibility: Visibility, familyId?: string | null) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Non autenticato')
...
export async function copyList(id: string) {
  ...
  if (list.list_items?.length > 0) {
    const items = list.list_items.map(({ id: _id, list_id: _lid, ...item }: Record<string, unknown>) => ({
      ...item,
      list_id: newList.id,
    }))
